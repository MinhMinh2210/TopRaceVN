...
export default function PackagesPage() {
  ...
  const savePackage = async () => {
    ...
    try {
      ...
      setOpen(false);
      setEditingPkg(null);
      alert(editingPkg ? '✅ Đã cập nhật gói!' : '✅ Đã thêm gói mới!');
      // Không tự load lại
    } catch (err: any) {
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={() => { setEditingPkg(null); setOpen(true); }}>
            + Thêm gói mới
          </Button>
        </div>
      </div>

      {errorMsg && <div className="mb-6 p-4 bg-red-900/30 border border-red-500 text-red-300 rounded-2xl">{errorMsg}</div>}
