...
export default function LeaderboardPage() {
  ...
  const loadData = async () => {
    ...
    // Sắp xếp + chỉ lấy run hợp lệ - ĐÃ SỬA CÚ PHÁP FILTER
    if (activeTab === 'speed') {
      query = query
        .order('max_speed', { ascending: false })
        .not('max_speed', 'is', null)     // ← SỬA Ở ĐÂY
        .gt('max_speed', 0);
    } else {
      query = query
        .order('zero_to_hundred', { ascending: true })
        .not('zero_to_hundred', 'is', null) // ← SỬA Ở ĐÂY
    ...
    if (error) {
      console.error('Lỗi load leaderboard chi tiết:', error); // giữ nguyên như code gốc
      setData([]);
      setLoading(false);
      return;
    }
